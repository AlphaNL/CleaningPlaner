...
form.addEventListener('submit', async (e)=>{
  e.preventDefault();
  const id = document.getElementById('clientId').value || (crypto.randomUUID ? crypto.randomUUID() : String(Date.now()));
  const client = {
    id,
...
function turnByPoint(px, py){
  ...
  const headX = (snake[0].x + 0.5) * (rect.width / N);
  const headY = (snake[0].y + 0.5) * (rect.height / N);
